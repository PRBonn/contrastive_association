...
import numpy as np
import time
from cont_assoc.utils.eval_np import Panoptic4DEval
...
class PanopticKitti4DEvaluator:
    def __init__(self, cfg):
        ...
        min_points = 50
        self.evaluator = Panoptic4DEval(self.nr_classes,
                                        None,
                                        self.ignore_class,
                                        offset=2**32,
    ...
    def update(self, sem_preds, ins_preds, inputs):
        for i in range(len(sem_preds)):
            self.evaluator.addBatch(inputs['pcd_fname'][i].split('/')[-3],
                                    sem_preds[i], ins_preds[i],
